...
class ViewSecond():
	...
	def paintGL(self, aspect_ratio):
		...
		if self.scene.controls.show_light_camera:	
			...
			M = cam_mvp * glm.inverse(self.scene.light_view_camera.P * self.scene.light_view_camera.V)
		...
		if self.scene.controls.show_main_camera:
			...
			M = cam_mvp * glm.inverse(self.scene.main_view_camera.P * self.scene.main_view_camera.V)
			self.scene.prog_shadow_map['u_mvp'].write(M)
