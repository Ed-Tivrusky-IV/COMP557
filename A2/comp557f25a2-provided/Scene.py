...
class Scene:
    ...
    def get_ground_plane(self) -> glm.vec4:
        ...
        # why is it in world coordinates? because it was computed from the object mesh in world coordinates
    ...
    def get_light_pos_in_world( self ) -> glm.vec4:
        ...
        origin_in_light_view = glm.vec4(0,0,0,1) # light is at origin in light view
        light_in_world = glm.inverse( self.light_view_camera.V ) * origin_in_light_view
        return light_in_world
    ...
    def compute_nf_from_view(self, V: glm.mat4):
        ...
        verts = self.get_all_scene_verts()  # get all scene verts
        V_np = np.array(V)
        verts_in_view = V_np @ verts  # transform all scene verts to the view
        z_vals = -verts_in_view[2, :] / verts_in_view[3, :]  # compute the negative z values (i.e., distances along -Z axis)
        # print(np.min(z_vals), np.max(z_vals))
        n = np.min(z_vals[z_vals > 0])  # smallest positive z value
        f = np.max(z_vals)              # largest z value
    ...
    def compute_lrbt_for_projection(self, V: glm.mat4, n: float, f: float):
        ...
        projection_matrix = glm.mat4((n,0,0,0),(0,n,0,0),(0,0,n+f,-1),(0,0,n*f,0))
        verts = self.get_all_scene_verts()  # get all scene verts
        V_np = np.array(V)
        projection_matrix = np.array(projection_matrix)
        verts_in_view = projection_matrix @ V_np @ verts  # transform all scene verts to the view
        x_vals = verts_in_view[0, :] / verts_in_view[3, :]  # compute the normalized x values
        y_vals = verts_in_view[1, :] / verts_in_view[3, :]  # compute the normalized y values

        # compute l,r,b,t for the scene vertices, given the near and far values, and return these values!
        l = np.min(x_vals)
        r = np.max(x_vals)
        b = np.min(y_vals)
        t = np.max(y_vals)
    ...
    def render_shadow_pass(self):
        ...
        mvp = self.light_view_camera.P * self.light_view_camera.V # compute the appropriate matrix to use for rendering the shadow map for the light camera
        ...
        # compute the light space transformation matrix and send to GLSL program for rendering the main view
        window_to_texture = glm.mat4((0.5,0,0,0),(0,0.5,0,0),(0,0,0.5,0),(0.5,0.5,0.5,1))
        light_space_transform = window_to_texture * mvp
