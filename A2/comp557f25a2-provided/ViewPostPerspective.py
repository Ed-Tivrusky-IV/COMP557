...
class ViewPostPerspective():
	...
	def paintGL(self, aspect_ratio):
		...
		light_pos = self.scene.get_light_pos_in_view( self.scene.main_view_camera.V ) # light pos in main camera view
		...
		# OBJECTIVE: draw frustums and axes for this view as required by the assignment specification

		# draw the origin of the CCV frame 
		M = P * V # compute the appropriate matrix to draw the CCV axis for this view
		self.scene.prog_shadow_map['u_mvp'].write(M)
		self.scene.axis.render()

		if self.scene.controls.show_light_camera:		
			
			M = mvp * glm.inverse(self.scene.light_view_camera.P * self.scene.light_view_camera.V) # compute the appropriate matrix to draw the light camera frustum for this view
			self.scene.prog_shadow_map['u_mvp'].write(M)
			self.scene.prog_shadow_map['u_color'] = (1, 1, 0, 0.75) # make light frustum yellow
			self.scene.render_cube_and_grid()
			
			M = mvp * glm.inverse(self.scene.light_view_camera.V) # compute the appropriate matrix to draw the light camera axis for this view
			self.scene.prog_shadow_map['u_mvp'].write(M)
			self.scene.render_axis()

		if self.scene.controls.show_main_camera:			
			
			M = P * V # compute the appropriate matrix to draw the main camera frustum for this view
			self.scene.prog_shadow_map['u_mvp'].write(M)
			...
			# I tried using M = mvp * glm.inverse(self.scene.main_view_camera.V) to draw the axis 
			# but the axis didn't show up in the view
			# that's because if we calculate it, the origin (0,0,0,1) will first multiply by P of the main camera
			# which will make the w component 0, and then when we do the perspective divide, it will be undefined
