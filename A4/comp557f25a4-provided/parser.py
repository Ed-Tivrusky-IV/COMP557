...
def load_scene(infile: str, image_scale_factor: float = 1.0) -> scene.Scene:
    ...
    for material in data["materials"]:
        ...
        mat_mirror = 0 if "mirror" not in material else material["mirror"]
        material_by_name[mat_name] = hc.Material(mat_id, mat_diffuse, mat_specular, mat_shininess, mat_mirror)
        mat_id += 1
    ...
    objects = {"sphere": [],
               ...
               "mesh": [],
               "cylinder": []
               }  # lists of loaded object geometries and hierarchy roots
    ...
    nb_rings = len(objects["ring"])
    rings = geom.Ring.field(shape=max(1, nb_rings))
    for i in range(len(objects["ring"])):
        rings[i] = objects["ring"][i]
    ...
    nb_cylinders = len(objects["cylinder"])
    cylinders = geom.Cylinder.field(shape=max(1, nb_cylinders))
    for i in range(len(objects["cylinder"])):
        cylinders[i] = objects["cylinder"][i]

    return scene.Scene( jitter, samples,  # General settings
                camera,  # Camera settings
                ambient, lights, nb_lights,  # Light settings
                spheres, nb_spheres,
                cylinders, nb_cylinders,
                planes, nb_planes,
                rings, nb_rings,
                boxes, nb_boxes,
...
def load_geometry(geometry, material_by_name, M_parent: tm.mat4 ):
    ...
    if g_type == "sphere":
        ...
        texture_id = geometry.get("texture_id", -1)
        return geom.Sphere(geom_id, g_materials[0], texture_id, g_radius, M, M_inv)
    elif g_type == "plane":
        ...
        g_inner_radius = geometry.get("inner_radius",1)
        g_outer_radius = geometry.get("outer_radius",2)
        g_normal = tm.vec3(geometry.get("normal",[0,1,0]))
        M, M_inv = load_geometry_transformation_matrix(geometry, M_parent)
        return geom.Ring(geom_id, g_materials[0], g_inner_radius, g_outer_radius, g_normal, M, M_inv)
    elif g_type == "box":
        ...
        g_radius = geometry.get("radius",1)
        g_height = geometry.get("height",1)
        M, M_inv = load_geometry_transformation_matrix(geometry, M_parent)
        return geom.Cylinder(geom_id, g_materials[0], g_radius, g_height, M, M_inv)
    else:
...
def load_instance(geometry, node_by_name):
    ...
    for obj_type, obj in node:
        if obj is not None:
            if obj_type == "sphere":
                new_obj = geom.Sphere(obj.id, obj.material, obj.position, obj.radius, M @ obj.M, obj.M_inv @ M_inv, obj.texture_id )                                      
            elif obj_type == "plane":
                new_obj = geom.Plane(obj.id, obj.two_materials, obj.material1, obj.material2, obj.position, obj.normal, M @ obj.M, obj.M_inv @ M_inv )   
            elif obj_type == "ring":
                new_obj = geom.Ring(obj.id, obj.material, obj.inner_radius, obj.outer_radius, obj.normal, M @ obj.M, obj.M_inv @ M_inv )         
            elif obj_type == "box":
                ...
                new_obj = geom.Cylinder(obj.id, obj.material, obj.radius, obj.height, M @ obj.M, obj.M_inv @ M_inv )                       
