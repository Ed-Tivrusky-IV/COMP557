...
class Vertex:
    ...
    def compute_Q(self):
        ...
        while True:
            f = h.face
            n = f.get_normal()
            d = -glm.dot(n, f.he.head.pos)
            # Plane equation: n.x + d = 0
            # Quadric contribution: Kp = [n.x n.y n.z d]^T * [n.x n.y n.z d]
...
class EdgeCollapseData:
    ...
    def __init__(self, he: HalfEdge):
        ...
        self.Q = he.head.Q + he.tail().Q
        self.v_opt = glm.vec3(0)
        midpoint = (he.head.pos + he.tail().pos) / 2.0
        Q_reg = glm.mat4((1, 0, 0, -1 * midpoint.x),
                         (0, 1, 0, -1 * midpoint.y),
                         (0, 0, 1, -1 * midpoint.z),
                         (-1 * midpoint.x, -1 * midpoint.y, -1 * midpoint.z, glm.dot(midpoint, midpoint)))
        Q_prime = self.Q + 0.0001 * Q_reg
        A = glm.mat3(Q_prime)
        b = -glm.vec3(Q_prime[0][3], Q_prime[1][3], Q_prime[2][3])
        try:
            self.v_opt = glm.inverse(A) * b
        except:
            self.v_opt = midpoint
            print("For half-edge with head vertex " + str(he.head.index) + " and tail vertex " + str(he.tail().index))
            print("A is not invertible, using midpoint instead")
        # else:
        #     print("A is invertible")
        # finally:
        #     print("Q_prime:\n" + str(Q_prime))
        #     print("A:\n" + str(A))
        v_opt_hom = glm.vec4(self.v_opt, 1)
        self.cost = glm.dot(v_opt_hom, self.Q * v_opt_hom)
...
def build_heds(F: np.ndarray, vert_objs: list[Vertex]) -> tuple[list[HalfEdge], list[Face]]:
    ...
    #Objective 1: Build the half-edge data structure
    for fi in range(F.shape[0]):
        face = Face(index=fi)
        face_objs.append(face)
        he0 = HalfEdge(head=vert_objs[F[fi, 0]], face=face, twin=None)
        he1 = HalfEdge(head=vert_objs[F[fi, 1]], face=face, twin=None)
        he2 = HalfEdge(head=vert_objs[F[fi, 2]], face=face, twin=None)
        he0.next = he1
        he1.next = he2
        he2.next = he0
    ...
    edgedict = {}
    for he in he_list:
        tail_index = he.tail().index
        head_index = he.head.index
        key = (tail_index, head_index)
        twin_key = (head_index, tail_index)
        edgedict[key] = he
        if twin_key in edgedict:
            twin_he = edgedict[twin_key]
            he.twin = twin_he
            twin_he.twin = he
