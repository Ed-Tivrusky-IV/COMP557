...
class SimplificationViewer(QtOpenGL.QGLWidget):
    ...
    def collapse_will_be_bad(self, he: HalfEdge) -> bool:
        ...
        vA = he.head
        vB = he.tail()
        # collect 1-ring neighbors of vA
        neighbors_A = set()
        h = vA.he
        start_h = h
        while True:
            neighbors_A.add(h.tail().index)
            h = h.next.twin
            if h == start_h:
                break
        # collect 1-ring neighbors of vB
        neighbors_B = set()
        h = vB.he
        start_h = h
        while True:
            neighbors_B.add(h.tail().index)
            h = h.next.twin
            if h == start_h:
                break
        common_neighbors = neighbors_A.intersection(neighbors_B)
        if len(common_neighbors) > 2:
            print("Bad collapse: more than 2 common neighbors:", common_neighbors)
            return True  # more than 2 common neighbors, bad collapse

        # TODO: Objective 4: Check if collapsing this half-edge will create problems
        return False

    def swap_faces(self, current_index, last_index):
        temp = self.faces[current_index, :].copy()
        self.faces[current_index, :] = self.faces[last_index, :]
        self.faces[last_index, :] = temp

        temp_obj = self.face_objs[current_index]
        self.face_objs[current_index] = self.face_objs[last_index]
        self.face_objs[last_index] = temp_obj
        self.face_objs[current_index].index = current_index
        self.face_objs[last_index].index = last_index
    ...
    def collapse(self, he: HalfEdge, vstar: glm.vec3, cost: float = 0.0) -> HalfEdge:
        ...
        he_a = he.next.twin
        he_b = he.next.next.twin
        he_c = he.twin.next.twin
        he_d = he.twin.next.next.twin
        he_a.twin = he_b
        he_b.twin = he_a
        he_c.twin = he_d
        he_d.twin = he_c

        he.next.head.he = he_b
        he.twin.next.head.he = he_d
        ...
        # there's no need to adjust the half edges because the list has included all half edges at initialization
        # and we will not be adding or removing any half edges from the data structure
        he_index = he.face.index
        last_face_index = len(self.face_objs) - 1 - 2 * self.current_LOD
        if he_index != last_face_index:
            # swap faces in face_objs
            self.swap_faces(he_index, last_face_index)

        last_face_index -= 1
        he_twin_index = he.twin.face.index
        if he_twin_index != last_face_index:
            # swap faces in face_objs
            self.swap_faces(he_twin_index, last_face_index)
        #  No need to calculate new face normals nor centers, they will be done when draw_debug is called
        ...
        half_edges_to_be_updated = []
        h = new_vertex.he
        start_h = h
        while True:
            half_edges_to_be_updated.append(h)
            half_edges_to_be_updated.append(h.twin)
            h = h.next.twin
            if h == start_h:
                break
        for h in half_edges_to_be_updated:
            if h.edge_collapse_data is not None:
                self.sorted_edge_list.discard(h.edge_collapse_data)
            edge = EdgeCollapseData(h)
            self.sorted_edge_list.add(edge)

        self.sorted_edge_list.discard(he.next.edge_collapse_data)
        self.sorted_edge_list.discard(he.next.next.edge_collapse_data)
        self.sorted_edge_list.discard(he.twin.next.edge_collapse_data)
        self.sorted_edge_list.discard(he.twin.next.next.edge_collapse_data)
        self.sorted_edge_list.discard(he.twin.edge_collapse_data)
        ...
        affected_faces.append(he_a.face)
        h = he_a.next.twin
        while h != he_a:
            affected_faces.append(h.face)
            h = h.next.twin
        print("Number of affected faces:", len(affected_faces)) # No need to include he.face and he.twin.face as they are being removed
        
        old_faces = np.empty((0, 3)) # Nx3 array of old face indices before the collapse
        new_faces = np.empty((0, 3)) # Nx3 array of new face indices after the collapse
        for f in affected_faces:
            old_faces = np.vstack((old_faces, self.faces[f.index].copy()))
            new_faces = np.vstack((new_faces, self.faces[f.index].copy()))
            for i in range(3):
                if new_faces[-1, i] == he.head.index or new_faces[-1, i] == he.tail().index:
                    new_faces[-1, i] = new_vertex.index
